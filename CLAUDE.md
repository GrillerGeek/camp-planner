...
- `RealtimeProvider.tsx` wraps the trip subtree (via `app/dashboard/trips/[tripId]/layout.tsx`) and owns a single presence channel (`presence:${tripId}`) with exponential-backoff reconnect and `onAuthStateChange` cleanup on sign-out. **The presence channel is `private: true`** (SPEC-003b.1) — RLS policies on `realtime.messages` (migration 022) enforce trip membership for both subscribe and send. The provider awaits `supabase.realtime.setAuth()` before every (re)subscribe so the realtime server has the current JWT to authorize against. **The provider owns the presence callbacks and `channel.track()` calls** — Supabase Realtime throws if `.on("presence", ...)` is registered after `subscribe()`, so handlers must be attached before subscribe. The provider takes `profile` as a prop and exposes `{ channel, connectionStatus, presentUsers }` via context.
...
- **Feature clients (packing/grocery/tasks) create their own ad-hoc per-list channels** (e.g. `packing-${listId}`) — this is the canonical per-feature pattern, ad-hoc `supabase.channel(...)` directly, handlers attached before `.subscribe()` (Supabase silently drops late `.on()` calls). `PackingListClient.tsx` is the reference implementation. Do not force feature clients through the presence provider.
- `VisibilityRefresher.tsx` — mounted once inside `TripRealtimeShell`. Calls `router.refresh()` when the tab regains visibility after being hidden ≥ 30 s, so Server-Component data (the trip row itself, member roster) re-fetches after long backgrounding. Per-feature realtime channels keep their own data fresh; this closes the gap for data that has no postgres_changes subscription (SPEC-003b.2).
